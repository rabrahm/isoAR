...
C
      ...
      open(UNIT=9,file=YYout2,status='new')
      endif

      if(Age.gt.0.0d0)then
        agenew=Age
      elseif(Age.eq.0.0d0)then
        open(UNIT=7,file='YY.age',status='old')
        read(7,*,end=900,err=900)agenew
      else
...
C-------
  ...
  831 format(2HZ=,f8.6,3H Y=,f8.6,4H OS=,f4.2,6H l/Hp=,f8.6,8H [Fe/H]=,
     +f9.3,12H [Alpha/Fe]=,f5.2)
      ...
      iYYm=AYYfin(8,Nalp)
      write(9,841)AYYfin(7,Nalp),iYYm
...
c The output
      do 200 ipoint=1,iYYm
      write(9,chfmt)(YYfin(ii,ipoint,Nalp),ii=1,nump-3),
